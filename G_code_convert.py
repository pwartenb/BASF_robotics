...
class CNC:
    ...
    def __init__(self, port):
        ...
        if self.is_open:
            print('CNC: %s open' % self.ser.name)
        else:
            print('failed to open serial port')

    def _send_cmd(self, data, wait=True):
        """
        Send command to the CNC machine.

        Args:
            data (string): the command
            wait (bool): wait for response from the machine (ok) or not.
                If True, this function will block until the arm response "ok"
                If False, this function will not block here. But the command could be ignored if buffer is full.
        """
    ...
    def move_to(self, x=None, y=None, z=None, e=None, feedrate=2000, mode="G01", wait=True):
        """
